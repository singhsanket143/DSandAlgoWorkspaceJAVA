...
public class TestClass {
	...
	public static long income(int f0, int f1, int n) {
		...
		long y = fib(n) % (mod - 1);
		long result = (((mr(f0 + 1, x, mod)) % mod) * ((mr(f1 + 1, y, mod)) % mod)) % mod;
		return (result - 1) % mod;
	}

	public static long mr(long a, long b, long c) {
		if (b == 0) {
			return 1;
		}
		if (b % 2 == 0) {
			return mr((a * a) % c, b / 2, c);
		} else {
			return ((a % c) * (mr((a * a) % c, b / 2, c))) % c;
		}
	}
	...
	public static void multiply(long[][] A, long[][] M) {
		...
		long svalue = (((A[0][0]) * (M[1][0]))%mod + ((A[0][1]) * (M[1][1]))%mod)%mod;
